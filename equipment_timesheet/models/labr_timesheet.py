...
class laborTimeSheetLine(models.Model):
    _name = "labor.timesheet.line"
    labor_timesheet_id = fields.Many2one('labor.timesheet',)
    ...
    def check_date(self):
        for rec in self:
            if rec.date and rec.labor_timesheet_id.date_from and rec.labor_timesheet_id.date_to:
                if not (
                        rec.date >= rec.labor_timesheet_id.date_from and rec.date <= rec.labor_timesheet_id.date_to):
    ...
    currency_id = fields.Many2one('res.currency',string='Currency',related='labor_timesheet_id.currency_id',stroe=True)
...
class laborTimeSheet(models.Model):
    _name = "labor.timesheet"
    name = fields.Char()
    ...
    @api.depends('line_ids', 'line_ids.unit_cost', 'line_ids.total_cost')
    def compute_total_cost(self):
        for rec in self:
    ...
    line_ids = fields.One2many(comodel_name="labor.timesheet.line", inverse_name="labor_timesheet_id", string="Line", required=False, )
